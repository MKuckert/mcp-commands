...
- [ ] **Task 2: Add Cross-Compilation Makefile**
  - **Description:** Create a `Makefile` at the project root with targets: `all`, `build`, `test`, `clean`, `lint`, and `cross`. The `cross` target should build binaries for `linux/amd64`, `linux/arm64`, `darwin/amd64`, `darwin/arm64`, and `windows/amd64`. It should use `git describe` to set `main.serverVersion` dynamically.
