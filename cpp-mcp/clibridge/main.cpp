/**
 * @file main.cpp
 * @brief CLI Bridge MCP Server
 *
 * This server demonstrates how to create an MCP server, register tools and
 * resources, and handle client requests. Follows the 2024-11-05 basic protocol
 * specification.
 */
...
mcp::json hello_handler(const mcp::json &params,
                        const std::string & /* session_id */) {
  std::string name =
      params.contains("name") ? params["name"].get<std::string>() : "World";
...
int main() {
  ...
  // Start server
  std::cout << "Starting MCP server at " << srv_conf.host << ":"
            << srv_conf.port << std::endl;
